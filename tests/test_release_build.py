...
class ReleaseBuildTests(unittest.TestCase):
    def test_version_060_is_shared_by_app_package_and_installer(self):
    ...
    def test_release_tag_must_match_application_version(self):
        ...
        with self.assertRaisesRegex(RuntimeError, "v0.6.0"):
            build_source_release.validate_tag("v0.5.0")
        with patch.dict(os.environ, {"GITHUB_REF_TYPE": "tag", "GITHUB_REF_NAME": "v0.5.0"}):
            with self.assertRaisesRegex(RuntimeError, "v0.6.0"):
                build_release.validate_release_tag()

    def test_source_archive_rejects_an_uncommitted_version_bump(self):
        with patch("build_source_release.subprocess.check_output", return_value='__version__ = "0.5.0"\n'):
